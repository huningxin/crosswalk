...
void XWalkNodeRendererController::DidCreateScriptContext(
  ...
  // Execute microtasks after calls into script contexts.
...
void XWalkNodeRendererController::StartNodeProcess() {
  const base::CommandLine::StringVector& argv =
      base::CommandLine::ForCurrentProcess()->argv();
  int new_size = 0;
  int new_argc = 0;
  std::string node_process_switch("--");
  node_process_switch += std::string(switches::kProcessType) +
      std::string("=") + std::string(switches::kXWalkNodeProcess);
  // Filter out the "--type=node-process" switch.
  for (size_t i = 0; i < argv.size(); ++i) {
    if (argv[i] == node_process_switch)
      continue;
    new_argc++;
    new_size += (argv[i].length() + 1);
  }
  std::unique_ptr<char[]> args_mem;
  args_mem.reset(new char[new_size]);
  char* new_argv[new_argc];
  char* args_ptr = args_mem.get();
  int j = 0;
  for (size_t i = 0; i < argv.size(); ++i) {
    if (argv[i] == node_process_switch)
      continue;
    int len = argv[i].length() + 1;
    memcpy(args_ptr, argv[i].c_str(), len);
    new_argv[j++] = args_ptr;
    args_ptr += len;
  }
  //base::debug::WaitForDebugger(180, true);
  for (int i = 0; i < new_argc; i++) {
    printf("argv[%d] = %s\n", i, new_argv[i]);
  }

  ::node::Start(new_argc, new_argv);
}
